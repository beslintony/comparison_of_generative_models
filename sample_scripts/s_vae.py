...
def generate_bash_script(dataset, dataset_args, config_args, exp_no):
    script_name = f"VAE_{dataset}_exp_{exp_no}.sh"
    
    base_folder = f"/clusterhome/user/comparison_of_generative_models"
    sbatch_log_folder = f"{base_folder}/exps/logs"

    script_content = f"""#!/bin/bash
#SBATCH --job-name=VAE_{dataset}_exp_{exp_no}
#SBATCH --output={sbatch_log_folder}/VAE_{dataset}_exp_{exp_no}.out
#SBATCH --error={sbatch_log_folder}/VAE_{dataset}_exp_{exp_no}.err

export BASE={base_folder}
export SCRIPT_PATH=${{BASE}}/t1_vae.py
